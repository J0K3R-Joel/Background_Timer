...
class FileR:
	def __init__(self, base_directory: str = '', allowed_path_seperator: str = '\\', forbidden_path_seperator: str = '/'):
		"""
	...
	def creator(self, path: str, text: str = '') -> bool:
		...
		if os.path.exists(file_path):
			self._msg.info(f'Path {file_path} already exists, nothing to create.')
			return False
		...
		self._msg.good('Path {file_path} created!')
		return True
	...
	def reader(self, file_name: str, type_name: str = 'str'):
		...
		try:
			...
			self._msg.error(file_path + ': ' + str(e))
	...
	def remover(self, file_name: str) -> None:
		...
		if not os.path.exists(file_path):
			self._msg.info(f'File {file_path} does not exist, nothing to remove')
			return

		os.remove(file_path)
		self._msg.good(f'Successfully removed file: {file_path}')
	...
	def transfer(self, from_file: str, to_file: str, create_to_transfer_file: bool = False) -> None:
		...
		if not os.path.exists(file_path_from):
			self._msg.bad(f'File {file_path_from} does not exist')
			return

		if not os.path.exists(file_path_to) and not create_to_transfer_file:
			self._msg.bad(f'File {file_path_to} does not exist')
			return

		text = self.reader(file_path_from)
		self.writer(file_path_to, 'w', text)
		self._msg.good(f'Successfully transferred text from {file_path_from} -> {file_path_to}')
